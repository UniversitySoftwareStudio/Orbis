...
class TEIProvider(EmbeddingProvider):
    ...
    def embed_text(self, text: str) -> List[float]:
        # Truncate to 500 chars to stay within MiniLM's token limit
    ...
    def embed_batch(self, texts: List[str]) -> List[List[float]]:
        try:
            ...
            if e.response is not None and e.response.status_code == 413:
                # Payload too large — fall back to one-at-a-time
                results = []
                for t in texts:
                    # Truncate individual text to 500 chars as safety net
                    truncated = t[:500] if len(t) > 500 else t
                    results.append(self.embed_text(truncated))
                return results
            raise
