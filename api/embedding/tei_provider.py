...
class TEIProvider(EmbeddingProvider):
    ...
    def __init__(self, api_url: str, model_name: str = "tei") -> None:
        self.api_url = api_url.rstrip("/")
        self.dimension = 384
        self.model_name = model_name
        self._validate_connection()

    def _post_embed(self, inputs: str | list[str], timeout: int) -> requests.Response:
        return requests.post(
            f"{self.api_url}{self.EMBED_PATH}",
            json={"inputs": inputs},
            timeout=timeout,
        )

    def _validate_connection(self) -> None:
        response = self._post_embed("connection test", timeout=5)
    ...
    def _embed_with_backoff(self, text: str, timeout: int = 10) -> list[float]:
        ...
        for max_chars in self.SAFE_CHAR_LIMITS:
            ...
            tried.add(candidate)
            try:
                response = self._post_embed(candidate, timeout=timeout)
                response.raise_for_status()
    ...
    def embed_batch(self, texts: list[str]) -> list[list[float]]:
        ...
        try:
            response = self._post_embed(short, timeout=30)
            response.raise_for_status()
