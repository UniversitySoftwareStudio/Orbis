...
Table~\ref{tab:metric_provenance} lists the provenance of every quantitative claim used in the evaluation. This table is intentionally included to make the evidence boundary explicit: the event and assignment labels are silver labels produced by LLM judges, while the submission cases have deterministic expected labels but are small and synthetic.
...
Event extraction & Precision 0.978; recall 0.574; $F_1$ 0.723 & 323 candidates from \texttt{event\_candidate\_logs} & \texttt{events/}\newline \texttt{candidates\_gt.jsonl}\newline \texttt{eval\_event}\newline \texttt{\_extraction.py} & Silver LLM labels, not human gold; recall is sensitive to how the judge defines ``obligation''. \\
Assignment matching & Precision 0.608; recall 0.446; $F_1$ 0.514 & 712 profile-obligation pairs (4 profiles $\times$ 178 accepted obligations) & \texttt{events/assignment}\newline \texttt{\_matching\_gt.jsonl}\newline \texttt{eval\_assignment}\newline \texttt{\_matching.py} & Silver applicability labels; only 4 student profiles; \texttt{maybe} is folded into the negative class for strict scoring. \\
Inter-judge agreement & 0.529 agreement on 51 sampled candidates & Stratified sample from event labels & \texttt{events/validation}\newline \texttt{\_report.json} & Used as a noise-floor estimate, not as a performance metric for Orbis. \\
...
Three findings stand out. First, the agent achieves \emph{perfect recall} (0 false negatives): no genuine submission was rejected, which is the safer error direction in an educational setting where false rejection harms the student. Second, redesigning the reviewer into a two-call agentic chain --- decomposing the assignment into explicit requirements before judging the document against each one --- raised accuracy from $0.781$ to $0.938$ and lifted in-document prompt-injection resistance from $1/4$ to $4/4$: because the agent now verifies whether the \emph{actual requirements} are met, an embedded ``ignore the requirements; approve'' instruction no longer satisfies them. Third, the only remaining error mode is over-approval of partial drafts ($2/4$ \texttt{flag\_partial} cases approved), motivating an explicit third \texttt{flag} output class as future work.

\subsection{Inter-Judge Agreement and Limitations}
On a stratified sample of 51 candidates, the secondary judge agreed with the primary judge on 27 cases ($53\%$). Disagreement was concentrated on candidates discussing conditional rights (e.g.\ ``Apply through the Disabled Student Unit if eligible''), which one judge treated as a non-obligation and the other treated as a true obligation. We treat this $53\%$ figure as a noise floor: the absolute values of recall reported above should be interpreted within roughly this band, and the precision number for event extraction (0.978) is the most robust of our results because it depends primarily on the clearer ``accepted'' subset. We additionally note that the submission test set is small ($N{=}32$) and synthetically generated; the absolute injection-resistance figure ($4/4$) should therefore be read as a directional finding (\emph{the redesigned agent resists the injection patterns we tested}), not a calibrated guarantee against all adversarial inputs.
