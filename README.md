...
### Embedding Models (Dynamic Load Balancer)

The root `docker-compose.yml` includes:
- `embedding-lb` (Traefik) as dynamic load balancer on `http://embedding-lb`
- TEI model services:
  - `embedding-e5` → `intfloat/multilingual-e5-small`
  - `embedding-bge-m3` → `BAAI/bge-m3`
  - `embedding-minilm` → `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`
  - `embedding-nomic` → `nomic-ai/nomic-embed-text-v1.5`
  - `embedding-all-minilm` → `sentence-transformers/all-MiniLM-L6-v2`

Local in-process embedding provider is removed. Embeddings are containerized only.

Run only one model family by enabling one profile:

```bash
# E5 model only
docker compose --profile e5 up -d embedding-lb embedding-e5

# BGE-M3 model only
docker compose --profile bge up -d embedding-lb embedding-bge-m3

# MiniLM model only
docker compose --profile minilm up -d embedding-lb embedding-minilm

# Nomic model only
docker compose --profile nomic up -d embedding-lb embedding-nomic

# All-MiniLM model only
docker compose --profile all-minilm up -d embedding-lb embedding-all-minilm
```

Set replica count at runtime (Compose scaling):

```bash
# Example: run 3 replicas of E5 behind the same LB
docker compose --profile e5 up -d --scale embedding-e5=3 embedding-lb embedding-e5
```

For API:

```ini
EMBEDDING_SERVICE_URL=http://embedding-lb
```
