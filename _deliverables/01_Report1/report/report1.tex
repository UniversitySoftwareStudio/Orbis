...
% Package Imports
\usepackage[T1]{fontenc}    % proper 8-bit font encoding (fixes Turkish glyphs)
\usepackage[utf8]{inputenc} % allow literal UTF-8 input (ı, ğ, ş, ö, ...)
...
% Allow long \texttt{} identifiers (e.g. jina-reranker-v2-base-multilingual,
% EventPipelineOrchestrator, requirement_findings) to break across lines
% instead of overflowing the right margin. seqsplit permits a break after
% every character within a monospace token.
\usepackage{seqsplit}
...
\begin{table}[H]
    ...
    \begin{adjustbox}{max width=\linewidth,center}
        \begin{tabular}{l c c p{5.5cm}}
            ...
            News \& Announcements & 4,158 & 33\% & Press releases, news items (\texttt{/haber/}) \\
            Events                & 4,032 & 32\% & Past and future seminars (\texttt{/etkinlik/}) \\
...
\begin{table}[H]
    \centering
        \begin{tabular}{l c l}
            \toprule
            \textbf{Category} & \textbf{Count} & \textbf{Feature Tested} \\ \midrule
            Direct Course Inquiries   & 20 & \texttt{sql} routing, exact field lookup \\
            Bulk Course Inquiries     & 10 & \texttt{sql} + CSV Compacting \\
            Personal Schedule         &  5 & \texttt{student\_schedule} SIS injection \\
            Academic Calendar         &  5 & \texttt{calendar} SIS injection \\
            Academic Regulations      & 25 & \texttt{vector} + Smart Context Expansion (PDFs) \\
            Campus Life \& Facilities & 20 & \texttt{vector} retrieval via web pages \\
...
\centering
\begin{tabular}{l|cc}
System decision $\backslash$ Gold & True obligation & Non-obligation \\ \hline
Accept & 174 & 4 \\
Reject & 129 & 16 \\
\end{tabular}
